...
/*
A k-means implementation that is generic across variable data dimensions.
*/
namespace kmeans {
...
//template <typename T>
//T distance_squared(const std::vector<T>& point_a, const std::vector<T>& point_b) {
//	T d_squared = T(), n1 = T(), n2 = T();
//	for (typename std::vector<T>::size_type i = 0; i < point_a.size(); ++i) {
//		d_squared += point_a[i]*point_b[i];
//		n1 += point_a[i]*point_a[i];
//		n2 += point_b[i]*point_b[i];
//	}
//	return (-d_squared*d_squared)/(n1*n2);
//}
//
//template <typename T>
//T distance(const std::vector<T>& point_a, const std::vector<T>& point_b) {
//	return 1.0 - std::sqrt(-distance_squared(point_a, point_b));
//}
...
*/
template <typename T>
void random_plusplus(const std::vector<std::vector<T>>& data, std::vector<std::vector<T>>& means, uint32_t k, uint64_t seed) {
	assert(k > 0);
	assert(data.size() > 0);
...
void calculate_means(const std::vector<std::vector<T>>& data,
	...
	uint32_t k) {
	for(auto& it:means) fill(it.begin(), it.end(), 0.);
...
*/
template <typename T>
void random_plusplus_parallel(const std::vector<std::vector<T>>& data, std::vector<std::vector<T>>& means, uint32_t k, uint64_t seed) {
	assert(k > 0);
	assert(data.size() > 0);
	using input_size_t = typename std::vector<T>::size_type;
	// Using a very simple PRBS generator, parameters selected according to
