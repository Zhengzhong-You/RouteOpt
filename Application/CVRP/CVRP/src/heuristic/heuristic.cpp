...
RE:
  ...
  completeHeuristic<false>(cvrp, node);
    ...
    symmetry_prohibit_call(auto &label_array_in_backward_sense = cvrp->label_array_in_backward_sense;)
    ...
    if (status == 3) {
        fake_ub *= LOWER_FAKE_UB_RATIO;
        cout << "lower the fake_ub by " << LOWER_FAKE_UB_RATIO << " and get " << fake_ub << endl;
        goto RE;
    }
...
void Heuristic::heuristicMIP(CVRP *cvrp, BbNode *&node) {
    ...
    mat.setFromTriplets(triplet.begin(), triplet.end()); {
        ...
        heuristicApplyRCF(local_solver, HEURISTIC_DELUXING_ROUND, true, node->getCurrentNodeVal() + fake_ub,
                          cvrp->real_dim, col_map);
#else
	col_map.resize(ccnt);
	iota(col_map.begin(), col_map.end(), 0);
#endif
