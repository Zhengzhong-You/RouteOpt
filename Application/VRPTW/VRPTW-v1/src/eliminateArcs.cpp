...
void CVRP::runLabelingForArcElimination(BbNode *const node) {
  ...
  if (rollback == ROLL_BACK_LONG_TIME || rollback == ROLL_BACK_LACK_MEMORY) goto QUIT;
  runLabeling<true, true, false, 0>(node);
  if (rollback == ROLL_BACK_LONG_TIME || rollback == ROLL_BACK_LACK_MEMORY) goto QUIT;
  concatenatePhaseInArcElimination<false, false>(node);
  if (rollback == ROLL_BACK_LONG_TIME || rollback == ROLL_BACK_LACK_MEMORY) goto QUIT;
  runLabeling<false, true, false, 0>(node);
  if (rollback == ROLL_BACK_LONG_TIME || rollback == ROLL_BACK_LACK_MEMORY) goto QUIT;
#else
  concatenatePhaseInArcElimination<true, true>(node);
  if (rollback == ROLL_BACK_LONG_TIME || rollback == ROLL_BACK_LACK_MEMORY) goto QUIT;
  runLabeling<true, true, true, 0>(node);
  if (rollback == ROLL_BACK_LONG_TIME || rollback == ROLL_BACK_LACK_MEMORY) goto QUIT;
#endif
