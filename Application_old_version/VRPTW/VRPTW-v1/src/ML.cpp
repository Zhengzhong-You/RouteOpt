...
void ML::collect_edge_related_features(BbNode *node, double org_val) {
  ...
  auto &MainResourceAcrossArcsInBackwardSense = cvrp->resource_across_arcs_in_backward_sense;
#endif
...
#ifdef SYMMETRY_PROHIBIT
	auto aver_res = (resource_across_arcs_in_forward_sense[edge.first][edge.second]
		+ MainResourceAcrossArcsInBackwardSense[edge.first][edge.second]).first_res / 2;
	e.basic_features.emplace_back("EdgeRes_ratio", aver_res / max_main_resource);
#else
	e.basic_features.emplace_back("EdgeRes_ratio",
								  resource_across_arcs_in_forward_sense[edge.first][edge.second].first_res
									  / max_main_resource);
#endif
  ...
  safe_solver(node->solver.getX(0, num_col, is_in_solution.data()))
}
